...
For agent role definitions, see `workflows/_shared/agent-team.md`.
For worktree setup mechanics (branch naming, base resolution, cleanup), see `workflows/_shared/worktree.md`. This phase invokes `./scripts/setup-sprint.sh` which implements that protocol.
...
6. **Create repository worktrees**: run `./scripts/setup-sprint.sh --config runs/sprint/{sprint-id}/sprint-config.yaml`. The script loops over the sprint-config `repositories` map; for each role:
...
7. **Repository sync (optional)**: run `./scripts/sync-repos.sh --config runs/sprint/{sprint-id}/sprint-config.yaml`. This only runs `git fetch origin {base}` in each source repo (sprint branches are not touched). Skip if the network is unavailable or you're in a read-only environment.

## Gate → Next Phase

**type=standard → Phase 2**:
- [ ] `runs/sprint/{sprint-id}/` directory layout complete (PRD.md, sprint-config.yaml, tasks/, contracts/, evaluations/, checkpoints/, logs/)
...
- [ ] `runs/sprint/{sprint-id}/` directory layout complete (sprint-config.yaml, qa-fix/, checkpoints/, logs/)
...
Sprint initialized: {sprint-id}
  Directory: runs/sprint/{sprint-id}/
  PRD: {prd-file}
