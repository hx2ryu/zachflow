...
### Changed
- **Teammate/workflow prompts tuned for Claude Opus 4.8.** A review against Anthropic's Opus 4.8 prompting best practices confirmed the harness needs no overhaul — prompts carry no hardcoded model references and run well as-is. Four targeted, guidance-backed adjustments landed: (1) the Evaluator and Adversarial Evaluator gain a "Coverage first, filter second" calibration that separates the finding stage (report every issue with a severity tag) from the verdict gate (PASS/ISSUES/FAIL filtering), countering Opus 4.8's more faithful following of "be conservative" review framing that can suppress real lower-severity findings; (2) `workflows/_shared/agent-team.md` adds a § Parallel Dispatch note so independent dispatches (prototype variants, non-overlapping per-task engineers) fan out in one turn, since Opus 4.8 spawns subagents more conservatively by default; (3) `workflows/_shared/kb-integration.md` states that a recalled/guessed pattern does not substitute for an actual `zachflow-kb:read`, countering the model's preference for reasoning over tool calls. The Design Engineer's existing Pass 6 Anti-Slop Audit already blocks the Opus 4.8 default-aesthetic leak (raw hex outside `tokens.css` is rejected), so no design-prompt change was needed. New [`docs/opus-4-8-notes.md`](docs/opus-4-8-notes.md) documents the recommended session settings (effort `xhigh`, adaptive thinking) — which are Claude Code session/API concerns, not harness markdown — and the expected behavior changes. Teammate `.template.md` mirrors updated in lockstep.
...
### Added
- **Three-agent-framework benchmark doc** — [`docs/benchmarks/2026-05-three-agent-frameworks.md`](docs/benchmarks/2026-05-three-agent-frameworks.md) captures a structured read of hermes-agent (skill curator, FTS5 sessions, role-gated ACL), ruflo (GOAP planner, HNSW vector store, ed25519 federation), and deer-flow (18-component middleware chain, SKILL.md `allowed-tools`, typed-fact memory). Records adoption decisions for seven portable ideas plus three skipped wholesale; the three landed in this release are tagged inline.
- **Pattern lifecycle curator — Phase 6.7b.** Pattern schema bumps to v2 with `state` (`draft | stable | archived`), `pinned`, `created_by`, `use_count`, `last_referenced_at`. `scripts/lib/curator.py` scans `logs/**/*.jsonl` + `runs/**/*.jsonl` for structured `pattern_id` references and applies two rules: `state == "draft" && use_count >= 3` → promote to stable; `state == "stable" && use_count == 0 && age > 90d` → archive (file moves to `learning/patterns/.archive/`). Manual mode (`--pattern-id ID --target-state STATE` / `--pinned true|false`) is wrapped by three new skills: `zachflow-kb:promote-pattern`, `zachflow-kb:archive-pattern`, `zachflow-kb:list-stale`. All transitions append to a kb-scoped hash-chained `logs/curator.jsonl`. `scripts/lib/migrate-pattern-v1-to-v2.py` is idempotent and seeds existing patterns with `state: stable`. Adopted from hermes-agent's `~/.hermes/skills/.usage.json` pattern.
- **Adversarial Evaluator teammate — T2-C.** `.claude/teammates/evaluator-adversarial.md` defines a read-only red-team teammate that auto-spawns after the standard Evaluator returns PASS for a group. Probes four spec-orthogonal surfaces — Security (auth / injection), Race & Concurrency, Malformed input (type / shape / size / encoding), Resource exhaustion — that conformance-only review cannot see by definition. Single pass per group (no re-spawn even when Fix Loop iterates). On Adversarial ISSUES the standard Fix Loop reenters with budget cap raised by exactly 1. Output is a separate file `evaluations/group-{N}.adversarial.md` so the audit trail of "who said what" stays clean. Probe reasoning is *read from code, not executed*, preserving the read-only constraint. `workflows/_shared/build-loop.md` gains §Adversarial Second Pass; `agent-team.md` registers the new role.
- **Four failure-mode guards — `docs/design-principles.md` §Failure Modes is now executable.** Each row of the 4-mode mapping table has a named script under `scripts/lib/guards/` invoked at a documented phase-transition boundary. `drift_guard.py` is the only producer of `block` (exit 1) — it scans the group branch's commit messages for admission phrases ("while I'm here", "also fix", "drive-by", ...); `--check-scope` adds warn-level out-of-Contract file detection. `self_deception_guard.py` (warn) checks same-author for impl ∩ eval, Evaluator commits touching source dirs, PASS verdicts with zero `file:line` citations. `context_guard.py` (warn) verifies the just-completed phase or group checkpoint exists, is non-anemic (≥ 100 bytes), has markdown headings; chain check when entering phase ≥ 4. `regression_guard.py` (warn) confirms active rubric clauses (`C\d+`) are referenced in the Contract and the `--- FROZEN SNAPSHOT ---` block is present. All four append hash-chained records to a project-scoped `logs/guards.jsonl` (verifiable via `jsonl-verify.py`). Inspired by deer-flow's `build_lead_runtime_middlewares()` — same intent (named interceptor per failure mode, typed events) translated into zachflow's markdown-harness shape.
- **`docs/design-principles.md` §Failure Modes section names the four LLM-agent failure modes the harness was designed against** (drift, self-deception, context exhaustion, recurring regressions), defines each, and maps them to the nine principles + the concrete mechanisms in `workflows/` and `docs/kb-system.md`. The Failure Modes table now also carries an "Automated guard" column linking to the four guard scripts. The "What the harness does NOT do" subsection pins the option-A/B boundary and acknowledges the three guards' partial coverage (e.g. the Adversarial Evaluator catches the false-PASS failure mode in four probe surfaces, but not the wrong-FAIL case).

### Changed
- **`docs/kb-system.md`** documents the Pattern Lifecycle (draft → stable → archived state machine, promote/archive rules, jsonl audit trail). Adds the three new lifecycle skills to the skills table.
- **`docs/roadmap.md`** marks Phase 6.7b, T2-C Adversarial Evaluator, and 4 failure-mode guards shipped. The remaining v1.x candidate from the benchmark is ed25519 audit signing (deferred — key management needs its own design pass).
- **`docs/logs-hash-chain.md`** registers `logs/curator.jsonl` (kb-scoped) and `logs/guards.jsonl` (project-scoped) as new producers alongside the per-sprint `events.jsonl` files.
- **`docs/structural-review-2026-05.md`** marks T2-C addressed inline + flips Quality Gates tree entries for `eval:security-review` and `eval:adversarial / red-team` from missing to present/partial.

### Fixed
- **`.gitignore` now excludes Python build artifacts** (`__pycache__/`, `*.pyc`, `*.pyo`). The first commit on the failure-mode-guards branch had an accidentally-tracked `.pyc` from local execution; a follow-up commit removed it and added the rule.
